...
const runScreenshotTest = async ({ explorerNodeId, fileName }) => {
  ...
  const container = await frame.waitForSelector(`[data-testid="sort"]`);
...
describe("Sort and its variants' appearance", () => {
  ...
  beforeAll(async () => {
    ...
    const storyGroupButton = await page.waitForSelector(`#disprz-disprzsort`);
  ...
  test("Standard", async () => {
    const explorerNodeId = "disprz-disprzsort--standard";
    const fileName = "standard";
    await runScreenshotTest({ explorerNodeId, fileName });
  });

  test("DefaultSelected", async () => {
    const explorerNodeId = "disprz-disprzsort--default-selected";
